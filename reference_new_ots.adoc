...
Cluster-Erweiterung:: ONTAP Select 9.13.1 führt eine Cluster-Erweiterung ein, die einen hochverfügbaren Schutz vor den Auswirkungen einer Node-Unterbrechung der Funktionen der Cluster zur Datenbereitstellung bietet. Ab ONTAP Select 9.13.1 können Sie ein Single-Node-Cluster zu einem Cluster mit zwei Nodes erweitern, indem Sie beide Nodes als HA-Paar konfigurieren. Das Verfahren wird automatisiert und über ONTAP Select Deploy über die CLI oder die Webschnittstelle gestartet. Um dieses Verfahren durchzuführen, müssen Sie bestimmte Hardware- und Softwareanforderungen erfüllen. link:concept_cluster_expansion.html["Weitere Informationen ."].
