...
---
= VMware ESXi externe Speicheranforderungen für ONTAP Select
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Lesen Sie die VMware-Dokumentation und befolgen Sie die darin beschriebenen Best Practices für ESXi-Hosts. Darüber hinaus:
...
* Stellen Sie sicher, dass alle generischen externen Speicherarrays die verfügbaren Redundanz- und HA-Funktionen nach Möglichkeit nutzen
