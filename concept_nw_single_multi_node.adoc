...
Der Netzwerkverkehr des Clusters muss über ein Layer-2-Netzwerk mit geringer Latenz und ohne Routing fließen. Aufgrund der Durchsatz- und Latenzanforderungen des Clusters wird erwartet, dass sich der ONTAP Select Cluster physisch in der Nähe befindet (z. B. Multipack, einzelnes Rechenzentrum).  Der Aufbau von Stretch-Cluster-Konfigurationen mit vier, sechs, acht, zehn oder zwölf Knoten durch die Trennung von HA-Knoten über ein WAN oder über große geografische Entfernungen wird nicht unterstützt. Eine gestreckte Zwei-Knoten-Konfiguration mit einem Mediator wird unterstützt.
