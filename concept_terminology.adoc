...
ONTAP Select Cluster:: Sie können einen _ONTAP Select Cluster_ erstellen, der aus einem, zwei, vier, sechs, acht, zehn oder zwölf Knoten besteht. Multi-Node-Cluster enthalten immer ein oder mehrere HA-Paare. Ein Cluster mit vier Knoten besteht beispielsweise aus zwei HA-Paaren. Ein Cluster mit einem Knoten bietet keine HA-Funktionalität.
...
Weitere Informationen finden Sie in den beiden Abschnitten link:concept_workflow_deploy.html["Planen"] und link:concept_lic_evaluation.html["Lizenz"].
...
Anmeldeinformationsspeicher:: Der Deploy-Anmeldeinformationsspeicher ist eine sichere Datenbank, die Anmeldeinformationen für Benutzerkonten enthält. Er wird hauptsächlich verwendet, um Hypervisor-Hosts im Rahmen der Erstellung eines neuen Clusters zu registrieren. Weitere Informationen finden Sie im link:concept_workflow_deploy.html["Planen"] Abschnitt.
...
+
--
Die Standardkonfigurationen von vNAS aktivieren eine Schreiboptimierungsfunktion namens Single Instance Data Logging (SIDL). Mit ONTAP Select 9.6 und späteren Versionen sind die Hintergrundfunktionen zur Speichereffizienz von ONTAP mit aktiviertem SIDL qualifiziert. Weitere Informationen finden Sie im link:concept_stor_concepts_chars.html["Im Detail"] Abschnitt.
