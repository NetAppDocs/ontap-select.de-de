...
NOTE: Wenn eine Instanz des ONTAP Select Deploy Administration Utility installiert ist, sollten Sie ein Upgrade auf die aktuelle Version in Betracht ziehen. Der ONTAP Select Node und die ONTAP Select Deploy Komponente werden unabhängig voneinander aktualisiert. Siehe link:concept_adm_upgrading_nodes.html["Aktualisieren der ONTAP Select-Knoten"^] Entnehmen.
