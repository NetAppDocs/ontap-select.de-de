...
NOTE: Die maximale Ausdehnung und Datenspeichergröße für ältere ESXi-Releases beträgt 64TB, was sich auf die Anzahl der LUNs auswirken kann, die zur Unterstützung der gesamten Bruttokapazität dieser großen Laufwerke erforderlich sind.
...
NOTE: Das Hinzufügen von Kapazität zu einer ONTAP Select VM führt wahrscheinlich zu VMDKs unterschiedlicher Größe. Details dazu finden Sie im Abschnitt link:concept_stor_capacity_inc.html["Erhöhung der Storage-Kapazität"]. Im Gegensatz zu FAS-Systemen können VMDKs unterschiedlicher Größe im selben Aggregat existieren. ONTAP Select verwendet einen RAID 0-Stripe über diese VMDKs, was die vollständige Nutzung des gesamten Speicherplatzes in jeder VMDK unabhängig von ihrer Größe ermöglicht.
...
NOTE: Die NVRAM-Partition ist auf einer eigenen VMDK getrennt. Diese VMDK ist mithilfe des vNVME-Treibers eingebunden, der in ESXi-Versionen ab 8.0 verfügbar ist. Diese Änderung ist besonders relevant für ONTAP Select Installationen mit Software-RAID, die nicht vom RAID-Controller-Cache profitieren.
