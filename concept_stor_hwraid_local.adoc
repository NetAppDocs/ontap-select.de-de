...
NOTE: Die NVRAM Partition ist auf einer eigenen VMDK getrennt. Diese VMDK wird über den vNVME-Treiber angebunden, der in ESX-Versionen ab 8.0 verfügbar ist. Diese Änderung ist besonders für ONTAP Select Installationen mit Software-RAID von Bedeutung, da diese nicht vom RAID-Controller-Cache profitieren.
