...
Obwohl das gespiegelte Aggregat vorhanden ist, um eine aktuelle (RPO 0)-Kopie des primären Aggregats bereitzustellen, ist es wichtig, dass im primären Aggregat nicht wenig freier Speicherplatz verfügbar ist. Ein geringer Speicherplatz im primären Aggregat kann dazu führen, dass ONTAP die gemeinsame Kopie von NetApp Snapshot™, die als Basis für das Storage-Giveback verwendet wurde, löscht. Dies funktioniert wie entwickelt, um Client-Schreibvorgänge zu ermöglichen. Da eine gemeinsame Snapshot Kopie beim Failback fehlt, muss der ONTAP Select Node jedoch eine vollständige Baseline vom gespiegelten Aggregat durchführen. In einer Umgebung ohne Shared-Ressourcen kann dieser Vorgang viel Zeit in Anspruch nehmen. Eine gute Ausgangsbasis für das Monitoring der aggregierten Speicherplatzauslastung ist um bis zu 85 %.
