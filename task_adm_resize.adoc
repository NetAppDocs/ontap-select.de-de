...
. Wählen Sie oben auf der Seite die Registerkarte *Cluster* aus und wählen Sie dann den gewünschten Cluster aus der Liste aus.
. Auf der Cluster-Detailseite das Zahnradsymbol rechts auf der Seite auswählen und *Instanzgröße ändern* wählen.
. Wählen Sie den *Instanztyp* aus und geben Sie die ONTAP Zugangsdaten ein, dann wählen Sie *Ändern*.
