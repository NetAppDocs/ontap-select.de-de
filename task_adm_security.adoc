...
. Wählen Sie das Figurensymbol oben rechts auf der Seite aus und wählen Sie dann *Passwort ändern*.
. Geben Sie das aktuelle und das neue Passwort wie aufgefordert ein und wählen Sie *Absenden*.
...
. Wählen Sie *Verwaltungsserver* > *vCenter hinzufügen*.
. Geben Sie die folgenden Informationen ein und wählen Sie *Hinzufügen*.
+
[cols="30,70"]
|===
...
|===
. Optional können Sie image:icon_kebab.gif["Optionen"] nach dem Hinzufügen des neuen Verwaltungsservers eine der folgenden Optionen auswählen:
+
