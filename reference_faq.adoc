...
Ja. ONTAP Select Deploy kann entweder auf KVM oder ESXi installiert werden, und beide Installationen können ONTAP Select Cluster auf beiden Hypervisoren erstellen.

.Ist vCenter für ONTAP Select auf ESXi erforderlich?
...
Wenden Sie sich an den NetApp technischen Support, wenn Sie einen ONTAP Select Node wiederherstellen müssen, beispielsweise wenn eine ONTAP Select VM oder ein Host vollständig verloren gegangen ist oder ein Single-Node-Cluster versehentlich gelöscht wurde. Weitere Informationen finden Sie im Knowledge Base Artikel link:https://kb.netapp.com/on-prem/ontap/ontap-select/Select-KBs/ONTAP_Select_node_VM_recovery_in_case_of_disaster["ONTAP Select Node VM-Wiederherstellung im Katastrophenfall"^].
...
Ja, Software RAID wird mit SSD-Medien sowohl auf KVM als auch auf ESXi unterstützt.
