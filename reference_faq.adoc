...
NOTE: Ab ONTAP Select 9.10.1 können Sie kein neues Cluster mehr auf dem KVM-Hypervisor implementieren. Bei ONTAP Select 9.11.1 und neueren Versionen sind alle Managementfunktionen für vorhandene KVM-Cluster und Hosts jetzt nicht mehr verfügbar, mit Ausnahme der Funktionen zum Offline- und Löschen. Die Fragen zu KVM sind nur auf frühere Versionen anwendbar, die KVM unterstützen.
