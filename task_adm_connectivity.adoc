...
. Wählen Sie oben auf der Seite die Registerkarte *Administration* und anschließend *Network Checker*.
. Wählen Sie *Neuen Lauf starten* und anschließend die Hosts und Netzwerke für das HA-Paar aus
+
Sie können bei Bedarf weitere HA-Paare hinzufügen und konfigurieren.

. Wählen Sie *Start*, um den Netzwerkverbindungstest zu starten.
