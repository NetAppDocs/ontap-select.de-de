...
NOTE: Ähnlich wie interner Datenverkehr kann auch externer Datenverkehr auf der vSwitch-Schicht (VST) und auf der externen Switch-Schicht (EST) gekennzeichnet werden. Zusätzlich kann der externe Datenverkehr von der ONTAP Select VM selbst in einem als VGT bekannten Prozess gekennzeichnet werden. Weitere Details finden Sie im Abschnitt link:concept_nw_data_mgmt_separation.html["Trennung des Daten- und Management-Datenverkehrs"].
